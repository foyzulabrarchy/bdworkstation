...
<body>
    ...
    if ($_SESSION['logged_in'] != 1) {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $package_price_sql = $mysqli->query("SELECT package_price FROM promotion_package WHERE id='$package_id'");
            while ($row = $package_price_sql->fetch_assoc()) {
                $package_price = $row['package_price'];
            }
            $post_data['total_amount'] = $package_price;
            $post_data['currency'] = "BDT";
            $post_data['tran_id'] = "promo_bdworkstation_" . uniqid();
            $transaction_id = $post_data['tran_id']; //work db
            $_SESSION['transaction_id'] = $transaction_id;
            $post_data['success_url'] = "http://localhost/bdworkstation/buyPromotion.php?success";
            $post_data['fail_url'] = "http://localhost/bdworkstation/buyPromotion.php?failed";
            $post_data['cancel_url'] = "http://localhost/bdworkstation/buyPromotion.php?cancelled";
